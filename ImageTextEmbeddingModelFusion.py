...
class ImageTextEmbeddingModelFusion(MyModel):
    def __init__(self, text_model_name, img_size, my_trainable=False, optimizer='adam', txt_dropout=True):
        ...
        y=layers.Concatenate()([img, txt, img_embedd])
        y=layers.Dense(2, activation='softmax')(y)

        self.model = tf.keras.Model(inputs=[text_input, img_input, img_embedd_input], outputs = [y])
